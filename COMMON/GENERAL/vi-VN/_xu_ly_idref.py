...
- Thêm tiền tố "G." vào giá trị data-id và vào phần text bên trong thẻ.

Ví dụ:
  <span id="idref" data-id="A1">A1</span>)
    -> <span id="idref" data-id="G.A1">G.A1</span>)

  <span id="idref" data-id="C2.1">C2</span>)
    -> <span id="idref" data-id="G.C2.1">G.C2</span>)

Lưu ý:
- Script tự động bỏ qua các thẻ đã có tiền tố "G." (để chạy lại nhiều lần
...
Cách chạy:
  python xu_ly_idref.py                # chạy thật, có backup
  python xu_ly_idref.py --dry-run       # chỉ xem trước, không sửa file
  python xu_ly_idref.py --no-backup     # chạy thật, không tạo file .bak
"""
...
def add_prefix(value: str) -> str:
    """Thêm tiền tố 'G.' vào value nếu chưa có sẵn."""
    value = value.strip()
